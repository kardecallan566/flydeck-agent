...
class SparseNetwork:
    ...
    def __init__(self, input_size: int, hidden_size: int, output_size: int, density: float = 0.15, seed: int = 42, input_plasticity: float = 0.0, recurrent_plasticity: float = 0.0) -> None:
        ...
        self._previous_decision_input = [0.0] * input_size
    ...
    def _connect(source_size: int, target_size: int, density: float, rng: random.Random) -> list[Connection]:
        ...
        connected_targets = {c.target for c in connections}
        for target in range(target_size):
            if target not in connected_targets:
                connections.append(Connection(rng.randrange(source_size), target, rng.uniform(-1.0, 1.0)))
        return connections
    ...
    def step(self, observation: tuple[float, ...]) -> tuple[float, ...]:
        ...
        self._previous_decision_input = list(self._decision_input)
    ...
    def learn(self, action: int, reward: float, learning_rate: float = 0.02) -> None:
        ...
        for connection in self._output_connections:
            ...
            weight = max(-2.0, min(2.0, connection.weight + learning_rate * reward * activation * direction))
    ...
    def learn_td(self, action: int, reward: float, next_scores: tuple[float, ...], done: bool, learning_rate: float = 0.005, discount: float = 0.97, trace_decay: float = 0.85) -> float:
        ...
        decision_input = self._decision_input if done else self._previous_decision_input
        bootstrap = 0.0 if done else max(next_scores)
        current = sum(c.weight * decision_state[c.source] for c in self._output_connections if c.target == action)
        td_error = max(-1.0, min(1.0, reward + discount * bootstrap - current))
        decay = discount * trace_decay
        competitor_scale = 1.0 / max(self.output_size - 1, 1)

        updated_output = []
        new_output_eligibility = []
        for index, connection in enumerate(self._output_connections):
            ...
            weight = max(-2.0, min(2.0, connection.weight + learning_rate * td_error * trace))
        ...
        if self.input_plasticity > 0.0:
            updated_input = []
            new_input_eligibility = []
            for index, connection in enumerate(self._input_connections):
                trace = decay * self._input_eligibility[index] + decision_input[connection.source] * decision_state[connection.target]
                weight = max(-2.0, min(2.0, connection.weight + learning_rate * self.input_plasticity * td_error * trace))
        ...
        if self.recurrent_plasticity > 0.0:
            ...
            for index, connection in enumerate(self._recurrent_connections):
                ...
                weight = max(-2.0, min(2.0, connection.weight + learning_rate * self.recurrent_plasticity * td_error * trace))
    ...
    def reset(self) -> None:
        ...
        self._previous_decision_input = [0.0] * self.input_size
        self._eligibility = [0.0] * len(self._output_connections)
        self._input_eligibility = [0.0] * len(self._input_connections)
        self._recurrent_eligibility = [0.0] * len(self._recurrent_connections)
