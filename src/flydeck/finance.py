...
class SyntheticCryptoMarket:
    ...
    def __init__(self, length: int = 256, initial_price: float = 100.0, seed: int = 42) -> None:
        ...
        self.length, self.initial_price, self.seed = length, initial_price, seed

    def generate(self) -> tuple[Candle, ...]:
        rng = random.Random(self.seed)
        price, candles = self.initial_price, []
        regime, regime_steps, previous_return = rng.choice(self.REGIMES), 0, 0.0
        for _ in range(self.length):
            if regime_steps <= 0:
                regime, regime_steps = rng.choice(self.REGIMES), rng.randint(12, 48)
            if regime == "trend_up":
            ...
            log_return = max(-0.25, min(0.25, drift + persistence * previous_return + rng.gauss(0.0, volatility) + shock))
            open_price, close = price, price * math.exp(log_return)
            ...
            price, previous_return, regime_steps = close, log_return, regime_steps - 1
...
class CryptoTradingEnvironment:
    ...
    def __init__(self, candles: tuple[Candle, ...] | list[Candle], initial_capital: float = 10_000.0,
                 trade_fraction: float = 0.25, fee_rate: float = 0.001, trade_penalty: float = 0.0025,
                 invalid_action_penalty: float = 0.001, drawdown_penalty: float = 0.02,
                 window: int = 24, max_steps: int | None = None) -> None:
        ...
        if fee_rate < 0 or trade_penalty < 0 or invalid_action_penalty < 0 or drawdown_penalty < 0:
            raise ValueError("penalties and fee_rate must be >= 0")
        self.candles = tuple(candles)
        self.initial_capital, self.trade_fraction, self.fee_rate = initial_capital, trade_fraction, fee_rate
        self.trade_penalty, self.invalid_action_penalty, self.drawdown_penalty = trade_penalty, invalid_action_penalty, drawdown_penalty
        self.window = window
        self.max_steps = max_steps or len(self.candles) - window - 1
        if self.max_steps < 1:
            raise ValueError("max_steps must be >= 1")
        self._closes = tuple(c.close for c in self.candles)
        self._returns = tuple(self._closes[i] / self._closes[i - 1] - 1.0 for i in range(1, len(self._closes)))
        self._average_volume = tuple(sum(c.volume for c in self.candles[max(0, i - 20):i]) / max(1, min(20, i)) for i in range(len(self.candles)))
        self.reset()

    def reset(self) -> tuple[float, ...]:
        self.index, self.steps = self.window, 0
        self.cash, self.asset_units = self.initial_capital, 0.0
        self.peak_value, self.max_drawdown = self.initial_capital, 0.0
        self.trades, self.invalid_actions = 0, 0
    ...
    def step(self, action: int) -> StepResult:
        ...
        price, traded, invalid_action = self._closes[self.index], False, False
        if action == 1:
        ...
        reward = raw_return - (self.trade_penalty if traded else 0.0) - (self.invalid_action_penalty if invalid_action else 0.0)
    ...
    def episode_result(self, total_reward: float) -> FinancialEpisodeResult:
        return FinancialEpisodeResult(total_reward, self._portfolio_value,
            (self._portfolio_value / self.initial_capital - 1.0) * 100.0,
            self.max_drawdown * 100.0, self.trades, self.steps)
    ...
    def _observation(self) -> tuple[float, ...]:
        ...
        return (
            max(-1.0, min(1.0, period_return(1) * 20.0)), max(-1.0, min(1.0, period_return(3) * 10.0)),
            max(-1.0, min(1.0, period_return(6) * 7.0)), max(-1.0, min(1.0, period_return(12) * 5.0)),
            max(-1.0, min(1.0, period_return(24) * 3.0)), min(1.0, volatility(6) * 50.0),
            min(1.0, volatility(24) * 25.0), max(0.0, min(2.0, current.volume / max(avg_volume, 1.0))),
            range_position, self.position_ratio, self.cash / max(self._portfolio_value, 1e-12),
