...
def main() -> None:
    ...
    print("FlyDeck Agent - Synthetic Crypto V7.3")
    print("learning: sparse k-WTA + recurrent circuit + TD(lambda) + controlled internal plasticity")
    ...
    print("plasticity: input 0.010 | recurrent 0.050 | output 1.000")
    ...
    training = train_synthetic_crypto_v7(
        ...
        epsilon=0.30, epsilon_decay=0.99, min_epsilon=0.05,
        trade_penalty=0.0025, invalid_action_penalty=0.001, drawdown_penalty=0.02,
        discount=0.97, trace_decay=0.85,
        opportunity_margin=opportunity_margin, confidence_threshold=confidence_threshold,
    )
    ...
    training_diagnostics = [
        diagnose_policy(agent, SyntheticCryptoMarket(length=256, seed=seed).generate(), label=f"training market seed {seed}", max_steps=200)
        for seed in range(100, 105)
    ]
    training_summary = aggregate_diagnostics(training_diagnostics, "Training-distribution diagnostics (5 markets)")
    unseen_summary = diagnose_policy(agent, SyntheticCryptoMarket(length=256, seed=evaluation_seed).generate(), label="Unseen-market diagnostics", max_steps=200)

    print()
    print("=== V7.3 POLICY DIAGNOSTICS ===")
