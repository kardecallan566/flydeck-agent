...
def test_market_retina_rising_and_falling_motion_have_distinct_vertical_channels() -> None:
    retina = BNBMarketRetina(width=8, height=6)
    rising = retina.encode((100.0, 100.5, 101.0, 101.5, 102.0))
    falling = retina.encode((102.0, 101.5, 101.0, 100.5, 100.0))
    assert rising.directions[2] > rising.directions[3]
    assert falling.directions[3] > falling.directions[2]
...
def test_visual_circuit_round_trip(tmp_path) -> None:
    ...
    assert loaded.neurons == circuit.neurons
    assert loaded.edges == circuit.edges
    assert loaded.l1_inputs == circuit.l1_inputs
